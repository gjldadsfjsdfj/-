...
function updateStageLogic() {
    ...
    if (!isBossFight && currentStageData.type !== 'boss') {
...
function goToMenu() { gameState = 'menu'; activeUI = null; }
function goToVillage() { gameState = 'village'; activeUI = null; }
function goToStage() { 
    gameState = 'stage'; 
    gameTimer = 0; 
    player.x = 100; 
    player.y = STAGE_HEIGHT - GROUND_HEIGHT - 100; 
    activeUI = null; 
    if(stages[stage-1].type === 'boss' && stages[stage-1].bossSpawnTime === 0) {
        isBossFight = true;
        createBoss();
    }
}
